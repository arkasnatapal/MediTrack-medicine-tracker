...
class RealtimeService {
  ...
  connect() {
    ...
    urls.forEach((url) => {
      try {
        ...
        s.on('connect', () => {
          console.log(`⚡ Realtime Socket connected to [${url}]:`, s.id);
          this.resubscribeActiveChannels();
          this.notifyReconnectListeners();
        });

        s.on('connect_error', (err) => {
          console.warn(`⚡ Realtime Socket connect_error on [${url}]:`, err.message);
        });
  ...
  handleIncomingEvent(eventPayload) {
    ...
    console.log(`🔔 Dispatching Realtime Event [${eventPayload.type}]:`, eventPayload);
      ...
      this.sockets.forEach((s) => {
        if (s && s.connected) {
          s.emit('subscribe', { channels: channelsList }, (res) => {
            if (res?.subscribed) {
              console.log('📡 Subscribed channels acknowledged:', res.subscribed);
            }
          });
        }
      });
