  ...
  return (
    <div className="relative min-h-screen w-full bg-[#F8FAFC] dark:bg-[#0B0F17] overflow-hidden transition-colors duration-500 font-sans">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-indigo-500/10 blur-[120px] mix-blend-multiply dark:mix-blend-screen" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] rounded-full bg-teal-500/10 blur-[120px] mix-blend-multiply dark:mix-blend-screen" />
      </div>

      <div className="relative z-10 p-6 md:p-8 h-screen overflow-y-auto custom-scrollbar">
        <motion.div
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex items-center justify-between mb-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 transition-all group shadow-sm"
            >
              <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />
              <span className="font-bold text-sm">Back to Family</span>
            </button>
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider border border-slate-200 dark:border-slate-700">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            {/* Left Column: Profile Card (Sticky Sidebar) */}
            <motion.div variants={itemVariants} className="lg:col-span-3 lg:sticky lg:top-6 space-y-6">
              <div className="relative overflow-hidden rounded-[2rem] bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-xl">
                {/* Cover Gradient */}
                <div className="h-28 bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600" />
                
                <div className="px-6 pb-8 -mt-14 flex flex-col items-center text-center relative z-10">
                  <div className="relative mb-3">
                    <div className="w-28 h-28 rounded-[2rem] overflow-hidden border-4 border-white dark:border-slate-900 shadow-xl bg-white dark:bg-slate-900">
                      <UserAvatar user={member} className="h-full w-full text-4xl" />
                    </div>
                    {isOnline && (
                  ...
                  <h1 className="text-2xl font-black text-slate-900 dark:text-white mb-0.5 tracking-tight">
                    {member.name}
                  </h1>
                  <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-6 break-all">
                    {member.email}
                  </p>

                  <button
                    onClick={() => navigate(`/family/chat/${userId}`)}
                    className="w-full py-3 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm"
                  >
                  ...
                  <div className="w-full mt-8 space-y-3 text-left">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 mb-2">Details</p>
                    <InfoRow icon={Clock} label="Active" value={formatLastActive(member.lastActive)} />
                    {member.location && <InfoRow icon={MapPin} label="From" value={member.location} />}
                    {member.phoneNumber && <InfoRow icon={Phone} label="Mobile" value={member.phoneNumber} />}
                    <InfoRow icon={Calendar} label="Joined" value={new Date(member.createdAt).toLocaleDateString()} />
                  </div>

                  {/* AI Consent Toggle */}
                  <div className="w-full mt-8 pt-6 border-t border-slate-100 dark:border-slate-800/50">
                    <div className="flex items-center justify-between group">
                      <div className="flex items-center gap-3 text-left">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-500">Enable smart reminders</p>
                        </div>
                      </div>
            ...
            {/* Right Column: Content Grid (9 cols) */}
            <div className="lg:col-span-9 space-y-6">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                 ...
                 <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden flex flex-col h-full">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                      <div>
                        <h2 className="text-lg font-bold text-slate-900 dark:text-white">Today's Timeline</h2>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Real-time tracking for {new Date().toLocaleDateString()}</p>
                      </div>
                      <div className="p-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm">
                      ...
                      {medicationStatus && medicationStatus.length > 0 ? (
                        <div className="space-y-3">
                          {(showAllStatus ? medicationStatus : medicationStatus.slice(0, 4)).map((item, idx) => (
                            <div 
                              key={idx} 
                              className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-800 transition-all group"
                            >
                              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                                ...
                                {item.status === 'confirmed' ? <CheckCircle className="w-5 h-5" /> :
                                 item.status === 'dismissed' ? <AlertTriangle className="w-5 h-5" /> :
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  {(() => {
                                      ...
                                      if (item.status === 'confirmed') {
                                        ...
                                        statusClass = isLate ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400';
                           ...
                           {medicationStatus.length > 4 && (
                            <button
                                ...
                                className="w-full text-xs font-bold text-slate-500 hover:text-indigo-600 py-2 border-t border-slate-100 dark:border-slate-800 transition-colors"
                            >
                                {showAllStatus ? "Show Less" : "View All Activities"}
                            </button>
                           )}
                        </div>
                      ) : (
                        <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400">
                           ...
                           <p className="text-sm font-medium">No updates today</p>
                        </div>
                      )}
                    </div>
                 </motion.div>

                 {/* 3. Quick Stats (Right Half) */}
                 <div className="space-y-4">
                    <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-4 h-full">
                        <div className="sm:col-span-2">
                             <StatCard title="Total Medicines" value={stats?.totalMedicines || 0} icon={Pill} color="indigo" />
                        </div>
                        <StatCard title="Low Stock Alerts" value={stats?.lowStock || 0} icon={AlertTriangle} color="amber" alert={stats?.lowStock > 0} />
                        <StatCard title="Expired items" value={stats?.expired || 0} icon={Activity} color="rose" alert={stats?.expired > 0} />
                    </motion.div>
                 </div>
              </div>
              ...
              <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
                <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
                  <div>
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">Active Prescriptions</h2>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Full inventory and schedules</p>
                  </div>
                  <div className="p-3 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 shadow-sm">
...
const InfoRow = ({ icon: Icon, label, value }) => (
  <div className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
    ...
    <span className="text-sm font-bold text-slate-900 dark:text-white text-right ml-4 leading-tight">{value}</span>
  </div>
);
