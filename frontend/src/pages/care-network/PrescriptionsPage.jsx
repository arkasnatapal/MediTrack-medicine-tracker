      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl text-xs font-bold w-full sm:w-auto">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg transition ${filterType === 'ALL' ? 'bg-white dark:bg-slate-800 text-teal-600 dark:text-teal-400 shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
          >
            All ({prescriptions.length})
          </button>
          <button
            onClick={() => setFilterType('DIGITAL')}
            className={`px-3 py-1.5 rounded-lg transition ${filterType === 'DIGITAL' ? 'bg-white dark:bg-slate-800 text-teal-600 dark:text-teal-400 shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
          >
            Digital PDF ({digitalCount})
          </button>
          <button
            onClick={() => setFilterType('OFFLINE')}
            className={`px-3 py-1.5 rounded-lg transition ${filterType === 'OFFLINE' ? 'bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-400 shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
          >
            Offline Hardcopy ({offlineCount})
          </button>
        </div>
      </div>
