...
import {
  ...
  MessageSquare, Send, X, RefreshCw, UserCheck, ShieldAlert, Activity, Volume2
} from 'lucide-react';
...
export default function LiveKitCallModal({
  ...
  useEffect(() => {
    ...
    async function connectToLiveKit() {
      try {
        ...
        // 4. Attach Local Camera Video Track to Local Preview
  ...
  const handleSendMessage = async (e) => {
    ...
    if (sessionId) {
      try {
        await api.post(`/teleconsultations/${sessionId}/in-session-chat`, {
          sender,
          text: msgText,
        });
      } catch (err) {
  ...
  return (
    ...
    >
      {/* Hidden Audio element for remote participant audio track */}
      ...
      <div className="relative w-full max-w-5xl h-[85vh] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-lg text-slate-100 flex items-center gap-2">
                {callType === 'VIDEO' ? 'LiveKit Video Consultation' : 'LiveKit Voice Consultation'}
              ...
              <p className="text-xs text-slate-400">
                {connectionState === 'CONNECTED' ? `Connected • Duration: ${formatTime(callDuration)}` :
                 connectionState === 'CONNECTING' ? 'Connecting to LiveKit media server...' :
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Toggle Live In-Session Chat"
            >
          ...
          <div className="flex-1 relative flex items-center justify-center overflow-hidden">
            ...
            {connectionState === 'CONNECTING' && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/80 z-20">
                ...
                <h4 className="text-lg font-semibold text-slate-200">Securing LiveKit Connection...</h4>
                <p className="text-sm text-slate-400 mt-1">Verifying room authorization and initializing WebRTC stream</p>
              </div>
            )}
            ...
            {connectionState === 'ERROR' && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/90 z-20 p-6 text-center">
                ...
                <h4 className="text-xl font-bold text-slate-100">LiveKit Media Connection Failed</h4>
                <p className="text-sm text-rose-400 mt-2 max-w-md">{errorMessage || 'Unable to join the requested call room.'}</p>
                <button
                  onClick={handleEndCall}
                  className="mt-6 px-6 py-2.5 bg-rose-600 hover:bg-rose-500 text-white font-medium rounded-xl transition-all shadow-lg shadow-rose-950"
                >
                  Close Call Window
                </button>
              </div>
            ...
            {/* Remote Participant Placeholder when Video is Off or Voice Call */}
            {(!remoteStreamActive || callType === 'VOICE') && connectionState === 'CONNECTED' && (
              <div className="flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-xs text-slate-400 mt-1 flex items-center gap-1.5">
                  ...
                  {callType === 'VOICE' ? 'LiveKit Voice Only Mode' : 'Participant camera is currently off'}
                </p>
              </div>
            )}
              ...
              <div className="flex-1 p-4 overflow-y-auto space-y-3">
                {chatMessages.length === 0 ? (
                  <div className="text-center text-slate-500 text-xs py-8">
                    No messages yet in this call session.
                  </div>
                ) : (
        ...
        <div className="px-6 py-4 bg-slate-900 border-t border-slate-800 flex items-center justify-center gap-4">
          ...
          <button
            onClick={handleEndCall}
            className="p-4 rounded-full bg-rose-600 hover:bg-rose-500 text-white transition-all border border-rose-500 shadow-xl shadow-rose-950 flex items-center justify-center"
            title="End Call"
          >
