...
// --- Helper: Generate Intelligence Logic (Shared) ---
async function generateHealthIntelligence(userId, force = false) {
  ...
  // If data hasn't changed and not forced, return existing
  ...
  // 2. Cooldown Check (Only strictly enforced for manual refreshes usually, but good to check context)
  // For auto-generation (force=false), we usually check time before calling this function.
  // But inside here, we just care about generating if data changed or force is true.
...
// --- Controller: Get Latest Intelligence (With Auto-Run) ---
exports.getIntelligence = async (req, res) => {
  try {
    const userId = req.user.id;
    let snapshot = await IntelligenceSnapshot.findOne({ userId })
      .sort({ generatedAt: -1 });

    // Auto-Run Logic: If no snapshot OR snapshot is older than 24 hours, try to generate
    if (!snapshot) {
      // First time? Generate automatically
      try {
        const result = await generateHealthIntelligence(userId, false);
        snapshot = result.snapshot;
      } catch (err) {
        console.error("Auto-generation failed (first run):", err);
        // Fallback to empty if failed
        return res.status(200).json({ exists: false, message: "Could not generate initial intelligence." });
      }
    } else {
      // Periodic check
      const hoursSinceLast = (Date.now() - new Date(snapshot.generatedAt).getTime()) / (1000 * 60 * 60);
      if (hoursSinceLast >= 24) {
        try {
          ...
          if (result.updated) {
           ...
           console.error("Auto-generation failed (periodic):", err);
...
// --- Controller: Refresh Intelligence (Manual Trigger) ---
exports.refreshIntelligence = async (req, res) => {
  try {
    const userId = req.user.id;
    const { force } = req.body; 

    // Manual refresh might want to enforce 24h cooldown if data hasn't changed?
    // But generateHealthIntelligence handles "data_unchanged".
    // We can just call it.
    
    // Optional: If you want to STRICTLY enforce 24h cooldown even if data CHANGED for manual button:
    // (User requirement said "supposed it should run for after 24 hours by automatically".
    // Manual refresh usually bypasses time if data changed.)
    
    const result = await generateHealthIntelligence(userId, force);
