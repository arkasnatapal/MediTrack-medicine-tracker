...
/**
 * Initialize Socket.IO server instance on HTTP server
 */
const initRealtimeService = (httpServer) => {
  ...
  io.use((socket, next) => {
    try {
      ...
      console.warn('⚡ Realtime Socket auth error:', err.message);
      socket.user = null;
      next();
    }
  });
  ...
  io.on('connection', (socket) => {
    ...
    console.log(`🔌 Realtime Socket connected: ${socket.id} (User: ${userId})`);
    ...
    socket.on('subscribe', ({ channels }, callback) => {
      ...
      console.log(`📡 Care Backend Socket ${socket.id} subscribed to: [${subscribed.join(', ')}], rejected: [${rejected.join(', ')}]`);
...
/**
 * Broadcast event locally to connected sockets on this server
 */
const emitLocalDomainEvent = (eventPayload) => {
  ...
  if (roomsArray.length > 0) {
    ...
    console.log(`📡 Care Backend Local Broadcast [${eventPayload.type}] to rooms [${roomsArray.join(', ')}]:`, eventPayload.eventId);
  } else {
    io.emit('domain_event', eventPayload);
    console.log(`📡 Care Backend Local Broadcast [${eventPayload.type}] globally:`, eventPayload.eventId);
  }
};

/**
 * Cross-relay domain event to peer backend (Patient Backend port 5000)
 */
const relayToPeerBackend = (eventPayload) => {
  const peerUrl = process.env.PATIENT_BACKEND_URL || 'http://localhost:5000';
  axios.post(`${peerUrl}/api/realtime-relay/emit`, { eventPayload }, { timeout: 3000 })
    .catch(() => {});
};

/**
 * Emit a standardized domain event to target authorized channels and relay to peer backend
 */
